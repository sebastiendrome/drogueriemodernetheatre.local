...
if(isset($_POST['context']) && $_POST['context'] == 'homeBg'){
	$path = urldecode($_POST['path']);
	$replace = urldecode($_POST['replace']);
...
if(isset($_POST['context']) && $_POST['context'] == 'fileInsert'){

	$result_path = CONTENT.'_uploads/_XL/';
...
if(isset($_POST['context']) && $_POST['context'] == 'gallery'){
	...
	if( !preg_match($_POST['types']['resizable_types'], $ext) ){
		...
		// upload file and resize. Return the image url to be inserted on success, 0 on failure
		if( up_file($upload_dest) ){
			$result = update_gallery_file($gallery_file, 'add', $result_path);
			//$result = PROTOCOL.SITE.$result_path;
			$root_upload_dest = ROOT.$result_path;
			list($w, $h) = getimagesize($root_upload_dest);
			$resize_result = resize_all($root_upload_dest, $w, $h);
...
if(isset($_GET['saveGalleryChange'])){
	...
	if( $gallery_file = update_gallery_file($path, $action, $file, $position) ){
		...
		$result = '<p class="error">Error: Could not update gallery file: '.$path.' width action: '.$action.' for file: '.$file.'</p>';
	}
}
